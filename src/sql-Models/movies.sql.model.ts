...
export const movieSqlModel = {
    ...
    getMovieById: async (id: number) => {
        const [movie] = await pool.query(`select * from movies where movie_id = ?`, [id]);
        if (!movie) {
            throw new Error("No movie found");
        }
        return movie;
    },
    async createMovie(movie:{movie_name: string, movie_genre: string, rating: number}) {
        const [result]: any = await pool.query(`insert into movies(movie_name, movie_genre, rating) values(?,?,?)`, [movie.movie_name, movie.movie_genre, movie.rating]);
        const createdId = result.insertId;
        // const createdMovie= await this.getMovieById(createdId);
        // res.json(createdMovie);
        return {id:createdId, ...result};
    },
    async updateMovieById(id:number, movie:Partial<{
        movie_name:string,
        movie_genre: string,
        rating:Number
    }>){
        ...
        const updatedMovie= await pool.query(`update movies  set ${fields.join(", ")} where movie_id=?`,[...values, id]);
    ...
    async deleteMovieById(id:Number){
      const [deleteMovie]: any= await pool.query("delete from movies where movie_id=? ",[id]);
